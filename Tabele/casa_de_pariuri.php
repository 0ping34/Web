...
<head>
    <meta charset="UTF-8">
    <title>Casa de Pariuri</title>
    <link rel="stylesheet" type="text/css" href="../Styles/style3.css">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/PapaParse/5.3.0/papaparse.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
        function fillInputs(codp, nume, locatie, contact) {
...
<div class="inputContainer">
    <form method="post" action="">
        ...
        <input style="margin-bottom: 5px" type="text" id="locatie" name="locatie"><br>
        <label for="contact">Contact:</label>
        <input style="margin-bottom: 5px" type="text" id="contact" name="contact"><br>
    ...
    <table>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST['adaugare'])) {
                $sql = "INSERT INTO casa_de_pariuri (CODP, Nume, Locatie, Contact) VALUES ('" . $_POST["codp"] . "', '" . $_POST["nume"] . "', '" . $_POST["locatie"] . "', '" . $_POST["contact"] . "')";
                $conn->query($sql);
            } elseif (isset($_POST['modifica'])) {
                $sql = "UPDATE casa_de_pariuri SET Nume='" . $_POST["nume"] . "', Locatie='" . $_POST["locatie"] . "', Contact='" . $_POST["contact"] . "' WHERE CODP='" . $_POST["codp"] . "'";
                $conn->query($sql);
            } elseif (isset($_POST['sterge'])) {
                $sql = "DELETE FROM casa_de_pariuri WHERE CODP='" . $_POST["codp"] . "'";
                $conn->query($sql);
            }
        }

        $sql = "SELECT CODP, Nume, Locatie, Contact FROM casa_de_pariuri";
        if (isset($_POST['cauta']) && $_POST['cautare'] != '') {
            $sql .= " WHERE CODP='" . $_POST["cautare"] . "'";
        }
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr onclick='fillInputs(\"" . $row["CODP"] . "\",\"" . $row["Nume"] . "\",\"" . $row["Locatie"] . "\",\"" . $row["Contact"] . "\")'><td>" . $row["CODP"] . "</td><td>" . $row["Nume"] . "</td><td>" . $row["Locatie"] . "</td><td>" . $row["Contact"] . "</td></tr>";
            }
        } else {
