...
<head>
    ...
    <script>
        function fillInputs(coda, nume, prenume, varsta,codp) {
            document.getElementById('coda').value = coda;
...
<div class="inputContainer">
    <form method="post" action="">
        ...
        <input style="margin-bottom: 5px" type="text" id="coda" name="coda"><br>
    ...
    <table>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST['adaugare'])) {
                $sql = "INSERT INTO angajat (CODA, Nume, Prenume, Varsta,CODP) VALUES ('" . $_POST["coda"] . "', '" . $_POST["nume"] . "', '" . $_POST["prenume"] . "', '" . $_POST["varsta"] . "','" . $_POST["codp"] . "' )";
                $conn->query($sql);
            } elseif (isset($_POST['modifica'])) {
                $sql = "UPDATE angajat SET Nume='" .  $_POST["nume"]. "', Prenume='" .  $_POST["prenume"] . "', Varsta='" .  $_POST["varsta"] . "', CODP='" .  $_POST["codp"] . "' WHERE CODA='" .  $_POST["coda"]. "'";
                $conn->query($sql);
            } elseif (isset($_POST['sterge'])) {
                $sql = "DELETE FROM angajat WHERE CODA='" . $_POST["coda"] . "'";
                $conn->query($sql);
            }
        }

        $sql = "SELECT CODA, Nume, Prenume, Varsta, CODP FROM angajat";
        if (isset($_POST['cauta']) && $_POST['cautare'] != '') {
            $sql .= " WHERE CODA='" . $_POST["cautare"] . "'";
        }
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr onclick='fillInputs(\"" . $row["CODA"] . "\",\"" . $row["Nume"] . "\",\"" . $row["Prenume"] . "\",\"" . $row["Varsta"] . "\",\"" . $row["CODP"] . "\")'><td>" . $row["CODA"] . "</td><td>" . $row["Nume"] . "</td><td>" . $row["Prenume"] . "</td><td>" . $row["Varsta"] . "</td><td>" . $row["CODP"] . "</td></tr>";
            }
        } else {
